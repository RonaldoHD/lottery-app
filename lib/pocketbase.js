...
function getAuthStore() {
  ...
  if (authCookie) {
    try {
      let cookieValue = authCookie.split('=').slice(1).join('=').trim();
      ...
      // Try URL decoding first (PocketBase may URL encode the cookie)
      try {
      ...
      // Try parsing as JSON first (some PocketBase versions use plain JSON)
      try {
        ...
        try {
          ...
          console.error('Error parsing auth cookie (tried JSON and base64):', e2);
          return { isValid: false, model: null, token: null };
        }
      }
      ...
      authStoreCache = result;
      authStoreCacheTime = Date.now();
  ...
  return {
    ...
    admins: {
      authWithPassword: async (email, password) => {
        ...
        // Update auth cache immediately with the response data
        if (result?.token) {
          authStoreCache = {
            isValid: true,
            model: result.record || result.admin || result.model || null,
            token: result.token,
          };
          authStoreCacheTime = Date.now();
        }
        ...
        // Notify listeners
        const model = newStore?.model || result?.record || result?.admin;
        if (model) {
          authChangeListeners.forEach(callback => {
            try {
              callback(newStore?.token || result?.token, model);
            } catch (e) {
              console.error('Error in auth change listener:', e);
            }
          });
...
export async function createOrder(data) {
  const record = await apiRequest('create', {
    collection: 'orders',
    data: {
      // Basic ebook references
      ebook_id: data.ebook_id || '',
      // If you have a relation field to ebooks collection, reuse the same id
      ebooks: data.ebooks || data.ebook_id || '',
      // Customer info
      username: data.username || data.first_name || '',
      user_lastname: data.user_lastname || data.last_name || '',
      phone: data.phone || '',
    },
  });
  return record;
...
export async function getEbookByDrawId(drawId) {
  return apiRequest('getFullList', {
    collection: 'ebooks',
    filter: `draw_id = "${drawId}"`,
  });
